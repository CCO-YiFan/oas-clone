...
class DraftController extends Controller
{
    ...
    public function index(){
        ...
        OR programme_levels_id="2" ORDER BY EnglishName;');
        $undergraduate = DB::connection('mysql')->select('SELECT * FROM programmes WHERE programme_levels_id!="1" 
        AND programme_levels_id!="2" ORDER BY EnglishName');
        $semestersql = DB::connection('mysql')->select('SELECT * FROM semester_year_mappings
        INNER JOIN semesters on semester_year_mappings.semesters_id=semesters.id WHERE semester_year_mappings.year_offered=2022;
        ');
