...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if($application_status_log == null){
            ...
            $postgraduate = DB::connection('mysql')->select('select * from programme_picked p join programme_records r on p.programme_records_id=r.id join programmes pr on pr.id=r.programmes_id where p.application_records_id='.$applicationRecord->id);
    
                //$findprogrammerecord=ProgrammeRecord::where('id',$programmePicked1->programme_records_id);
               // $programme1 = Programme::where('id', $programmeRecord1->programmes_id)->get();
               return view('oas.home', compact('application_status_id','postgraduate'));
            }
            return view('oas.home', compact('application_status_id'));
        }
    }
